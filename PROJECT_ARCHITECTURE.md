...
本项目旨在开发一个智能旅游路线规划网站。用户可以根据个人需求，借助大语言模型（LLM）的智能规划能力，一键生成精确到时间点、地点、预算和交通方式的定制化旅游行程。系统支持路线在地图上的可视化展示，提供景点避坑指南，支持跨城大交通规划，并允许用户随时拖拽排序和重新规划。项目遵循企业级开发标准，保证代码质量、可扩展性和可维护性。

### 核心功能：
- **用户系统**：支持用户注册、登录、个人信息管理。
- **AI 智能生成**：根据用户填写的表单（出发地、目的地、预算、天数、偏好、跨城与市内交通偏好），调用大模型自动生成每日详细行程，包含花费预估和景点介绍。
- **行程动态编辑**：支持行程项的增删改，并深度集成 `@dnd-kit` 实现行程卡片的丝滑拖拽排序。
- **动态重规**：支持在原有参数基础上“就地更新”重新生成行程，避免数据库 ID 膨胀。
- **时间与大交通规划**：精准计算从出发地到目的地的跨城交通（飞机/高铁/火车）耗时及费用，并安排合理的市内游览顺序。
- **地图深度双向交互**：在高德地图上实时绘制行程路线与自定义图标（智能识别飞机/火车/景点）。支持点击时间轴卡片与地图视角的平滑联动（高亮、缩放居中），同时也支持点击地图标点反向自动滚动并高亮左侧时间轴。
